...
//        case cmd @ StartTransaction(_, DepositFunds(accountNumber, _, _)) =>
//          system.actorSelection(s"/user/$accountNumber") ! cmd
//        case cmd @ StartTransaction(_, WithdrawFunds(accountNumber, _, _)) =>
//          system.actorSelection(s"/user/$accountNumber") ! cmd
//        case cmd @ CommitTransaction(_, DepositFunds(accountNumber, _, _)) =>
//          system.actorSelection(s"/user/$accountNumber") ! cmd
//        case cmd @ CommitTransaction(_, WithdrawFunds(accountNumber, _, _)) =>
//          system.actorSelection(s"/user/$accountNumber") ! cmd
//        case cmd @ RollbackTransaction(_, DepositFunds(accountNumber, _, _)) =>
//          system.actorSelection(s"/user/$accountNumber") ! cmd
//        case cmd @ RollbackTransaction(_, WithdrawFunds(accountNumber, _, _)) =>
//          system.actorSelection(s"/user/$accountNumber") ! cmd
//      }
//    }))
//
...
//    case class BankAccountTransactionConfirmed(evt: TransactionalEvent)
//    case object GetEvents
//    case class GetEventsResult(confirms: Seq[BankAccountEvent], exceptions: Seq[BankAccountTransactionalExceptionEvent])
//    case object Reset
//    val eventReceiver: ActorRef = system.actorOf(Props(new Actor {
//      private var events: Seq[TransactionalEvent] = Seq.empty
//      private var exceptions: Seq[BankAccountTransactionalExceptionEvent] = Seq.empty
//      override def receive: Receive = {
...
//        case evt: BankAccountTransactionalEvent => eventReceiver ! BankAccountTransactionConfirmed(evt)
//        case ex: BankAccountTransactionalExceptionEvent    => eventReceiver ! BankAccountExceptionConfirmed(ex)
//      }
//
...
//      val source: Source[EventEnvelope, NotUsed] = readJournal.eventsByTag(TransactionId, Offset.noOffset)
//      source.map(_.event).runForeach {
//        case evt: TransactionalEvent => eventReceiver ! BankAccountTransactionConfirmed(evt)
//      }
//
